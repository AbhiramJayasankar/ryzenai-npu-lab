"""One compiled attention context program for cache lengths up to 64."""
...
SOURCE = Path(__file__).resolve().parent / "kernels" / "lfm25_attention_fixed64_head.cc"
CAPACITY = 64
...
@iron.jit
def attention_first_fixed64(qkv_and_hidden: In, packed_tail_input: Out,
                          first_cache: Out):
    ...
    op = ExternalFunction(
        "lfm25_attention_fixed64_first_head", source_file=str(SOURCE),
...
@iron.jit
def attention_context_fixed64(qkv_and_hidden: In, past_cache: In,
    ...
    op = ExternalFunction(
        "lfm25_attention_fixed64_head", source_file=str(SOURCE),
