...
Install a supported Ryzen AI release using [AMD's Windows instructions](https://ryzenai.docs.amd.com/en/1.7/inst.html), then confirm that a model actually runs on this Phoenix/Hawk Point NPU. AMD's guide calls for the `X1` target on this hardware. After that, measure a small model on CPU and NPU with the same inputs and record latency, throughput, and operator placement.
